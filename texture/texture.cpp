...
private:
	...
	void createDescriptorSets()
	{
		...
		for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
		{
			...
			imageInfo.imageView = texture_.view;				// The image's view (images are never directly accessed by the shader, but rather through views defining subresources)
			imageInfo.sampler = texture_.sampler;			// The sampler (Telling the pipeline how to sample the texture, including repeat, border, etc.)
			imageInfo.imageLayout = texture_.imageLayout;	// The current layout of the image (Note: Should always fit the actual use, e.g. shader read)
...
VkCommandBuffer createCommandBuffer(VkDevice logicalDevice, VkCommandBufferLevel level, VkCommandPool pool, bool begin)
{
...
void HelloTriangleApplication::loadTexture()
{
	string filename = "shaders/metalplate01_rgba.ktx";
	...
	image_create_info.extent = {texture_.width, texture_.height, 1};

	VK_CHECK_RESULT(vkCreateImage(device, &image_create_info, nullptr, &mappableImage));

	vkGetImageMemoryRequirements(device, mappableImage, &memReqs);
	memAllocInfo.allocationSize = memReqs.size;
	memAllocInfo.memoryTypeIndex = findMemoryType(memReqs.memoryTypeBits,
	...
	texture_.image = mappableImage;
	texture_.deviceMemory = mappableMemory;
	texture_.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

	VkCommandBuffer copyCmd = createCommandBuffer(device, VK_COMMAND_BUFFER_LEVEL_PRIMARY, commandPool, true);
	...
	image_memory_barrier.image = texture_.image;
	...
	flushCommandBuffer(device, copyCmd, graphicsQueue, commandPool, true);
	...
	VK_CHECK_RESULT(vkCreateSampler(device, &sampler, nullptr, &texture_.sampler));
